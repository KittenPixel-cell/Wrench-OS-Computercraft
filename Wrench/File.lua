...
Who = "By KittenPixel"
instaClose = true
Version = "Wrench File Manager v1"
timeX = ""
renderTime = true
dragLasY = 0
changepos = true
...
function startMain()
	...
	function fManRun(locationPath)
		if fs.exists(locationPath) == true then
			if openWith ~= "" then
				...
				parallel.waitForAny(runProgram, terminateKey)
				print("[Lemon] Press Any Key...")
				os.pullEvent("key")
				renderXX = true
				openWith = ""
	...
	parallel.waitForAny(render, input, guiController)

end

while true do
	ok, err = pcall(startMain)
	if not ok then
